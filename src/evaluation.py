...
class RacePredictor(nn.Module):
    ...
    def __init__(self, version="resnet34_7", device="cpu"):
...
class RaceDeltaLoss(nn.Module):
    def __init__(self, version="resnet34_7", loss="l2", device="cpu", verbose=False):
...
def compute_losses(orig_dir, recon_dir, labels_path):
    ...
    LOSS_FCTs = {"lpips": lpips.LPIPS().to('cpu'),
                 "ssim": SSIMLoss(1),
                 "race_cos": RaceDeltaLoss(loss="cosine", device='cpu'),
                 "race_0-1": RaceDeltaLoss(loss="0-1", device='cpu'),
    ...
    for batch in tqdm(dataloader):
        orig_img = batch[0].to('cpu')
        recon_img = batch[1].to('cpu')
        base_names = batch[2]
        races = [] 
        races_id = [] 
        files = [] 
...
def compute_diversity(orig_dir, recon_dir, labels_path, num_duplicates=100):
    ...
    loss = RaceDeltaLoss(loss="0-1", device='cpu')
